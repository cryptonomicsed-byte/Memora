# Sigil: Value Proposition

**Sigil is a cryptographically enforced supply-chain guardrail for dApp front-ends. It's a security and governance layer that sits on top of raw decentralized storage.**
...
Sigil lets agents ship to production **without being trusted**. The guardrails are enforced by the chain, not by the agent's behavior:
...
| DNS or registrar hijack points users elsewhere | Content is pinned by manifest root on-chain, and portals serve only bytes that hash to it | `sigil_verify_live` |
...
| Rollback takes minutes or hours mid-incident | One signer, one tx: `freeze_site` → `rollback` → `revoke` | `sigil_incident_response` |

### 2. Multi-portal access, honestly scoped

Files stored on Walrus or Arweave persist independently of any company. **Most people still reach a site through an HTTP portal** (for example, wal.app), and any single portal can be blocked or taken down. Sigil treats portals as a **redundant fallback network**, not a single front door:
...
Decentralized blob storage isn't permanent by default. Walrus storage is paid per epoch and expires if it isn't renewed. Sigil makes renewal programmatic:
...
Mysten's `walrus-sites` tooling handles storing and indexing static sites well. Sigil doesn't compete with that layer. It adds what that layer doesn't have:
...
## Part 2: The SIGIL token's role

SIGIL exists to make the review layer trustworthy and to account for how much the protocol is used. The description below is functional. It isn't a promise of price performance.

| Function | Mechanism | Where |
|---|---|---|
| **Security bond** | Sentinels must bond SIGIL to review builds. The 7-day unbonding window prevents approve-and-exit. Stake slashed for attesting to a revoked build is **burned**, not redistributed, so reviewers gain nothing from colluding to slash each other. | `sentinel_registry.move` |
| **Usage-based supply contraction** | Deploy, storage-renewal, and domain fees are paid in SUI and split on-chain (default 40% buyback, 30% portals, 20% Sentinels, 10% operations). A Keeper agent converts the buyback share to SIGIL and burns it through a vault that exposes no mint function. Contraction tracks actual protocol usage. | `treasury.move`, `sigil.move` |
...
| **Fee tiers** | Locking SIGIL reduces platform fees (20%, 40%, or 60% off) and unlocks priority routing and continuous monitoring. | `treasury::discount_bps` |
