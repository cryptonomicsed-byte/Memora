#!/usr/bin/env node
// Immutex MCP server (stdio). Wires the tool surface to a ledger and a blob store.
//
//   IMMUTEX_HOME        state dir (default ./.immutex): ledger snapshot, blobs, agent memory
//   IMMUTEX_STORAGE     "fs" (default) | "walrus"
...
//   IMMUTEX_LEDGER      "sim" (default) | "sui"
//   SUI_NETWORK        testnet (default) | mainnet | devnet
//   SUI_GRPC_URL       fullnode gRPC URL (default https://fullnode.<network>.sui.io)
//   IMMUTEX_PACKAGE_ID  published immutex package ID          (sui ledger)
//   IMMUTEX_REGISTRY_ID shared sentinel_registry::Registry   (sui ledger)
...
const home = resolve(process.env.IMMUTEX_HOME ?? ".immutex");
mkdirSync(home, { recursive: true });
const statePath = join(home, "ledger.json");

function requireEnv(name: string): string {
  const v = process.env[name];
  if (!v) throw new Error(`${name} is required when IMMUTEX_LEDGER=sui`);
  return v;
}
...
async function suiLedger(): Promise<SuiLedger> {
  ...
  return new SuiLedger({
    packageId: requireEnv("IMMUTEX_PACKAGE_ID"),
    registryId: requireEnv("IMMUTEX_REGISTRY_ID"),
...
const simulated =
  process.env.IMMUTEX_LEDGER === "sui"
    ? undefined
    : new SimulatedLedger(
...
const store: BlobStore =
  process.env.IMMUTEX_STORAGE === "walrus"
    ? new WalrusBlobStore({
...
const server = new McpServer({ name: "immutex", version: "0.1.0" });
