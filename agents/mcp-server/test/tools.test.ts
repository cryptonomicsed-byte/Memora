...
async function build(files: Record<string, string>) {
  const dir = await mkdtemp(join(tmpdir(), "immutex-build-"));
...
async function context() {
  ...
  const ctx: ToolContext = {
    ...
    memory: new AgentMemory(await mkdtemp(join(tmpdir(), "immutex-mem-"))),
    profilesDir: resolve(import.meta.dirname, "../../profiles"),
  };
  const { siteId } = await call(ctx, "immutex_create_site", {
...
async function ship(ctx: ToolContext, siteId: string, dir: string) {
  const d = await call(ctx, "immutex_deploy", { actor: "dev", site_id: siteId, dir });
  await call(ctx, "immutex_approve", { actor: "dao", site_id: siteId, proposal: d.proposal });
  const review = await call(ctx, "immutex_review_proposal", {
...
test("profiles are discoverable", async () => {
  ...
  const agents = await call(ctx, "immutex_list_agents", {});
...
test("clean build ships, injected drainer is blocked, incident rollback restores v0", async () => {
  ...
  await call(ctx, "immutex_promote", { actor: "dev", site_id: siteId, proposal: first.d.proposal });
  assert.equal((await call(ctx, "immutex_verify_live", { site_id: siteId })).ok, true);
  ...
  await assert.rejects(call(ctx, "immutex_promote", { actor: "dev", site_id: siteId, proposal: bad.d.proposal }), /EReviewsNotMet/);
  ...
  await call(ctx, "immutex_promote", { actor: "dev", site_id: siteId, proposal: second.d.proposal });
  const ir = await call(ctx, "immutex_incident_response", { actor: "dao", site_id: siteId, reason: "reported drain", memory_namespace: "guardian" });
  assert.deepEqual(ir, { frozen: false, rolledBackTo: 0, revoked: 1, slashEvidence: ["sentinel-1"] });
  const status = await call(ctx, "immutex_site_status", { site_id: siteId });
  assert.equal(status.liveResolved.blobId, first.d.blobId);

  const mem = await call(ctx, "immutex_recall", { namespace: "sentinel", query: "block", limit: 5 });
...
test("tampered blob is flagged on review", async () => {
  ...
  const r = await call(ctx, "immutex_review_proposal", {
